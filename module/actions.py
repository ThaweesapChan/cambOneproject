...
def screenShotLR():
    ...
    pyautogui.screenshot("/Users/thaweesap/CodingProject/cambOneProject/img/screenShot/pageLeft.png", region=(0, 0+(screen_height*0.22), screen_width / 2, screen_height))

    # Take a screenshot of the right side of the screen and save it to a file
    pyautogui.screenshot("/Users/thaweesap/CodingProject/cambOneProject/img/screenShot/pageRight.png", region=(screen_width / 2, 0+(screen_height*0.22), screen_width / 2, screen_height))
...
def pageCyclechoice():
    """
    1.จอซ้าย --> click choice --> click Check
    2. if คำตอบถูก --> คลิกคำตอบเดิมที่ด้านขวา --> คลิก check --> Next ทั้งสองข้าง
    3. if คำตอบผิด --> อ่านเฉลยด้านซ้าย --> หาคำตอบที่ตรงกันทางด้านขวา --> คลิก check --> Next ทั้งสองข้าง
    """
    start_time = time.perf_counter()
    while True :
        # screenShot หน้าจอ
        screenShotLR()
        clickDoubleObject(buttonCycle, pageLeft, "L")

        screenShotLR()
        clickDoubleObject(buttonCheck, pageLeft, "L")

        #หากการตอบถูกต้องให้ตอบคำตอบเดิมกับภาพทางด้านขวาและออกจาก while loop
        screenShotLR()
        if countObject(iconCorrect,pageLeft) > 0 :
            clickDoubleObject(buttonCorrectChoice, pageLeft, "R")
            screenShotLR()

            clickDoubleObject(buttonCheck, pageRight, "R")
            screenShotLR()

            if countObject(buttonNext,pageLeft) + countObject(buttonNext,pageRight) > 1 :
                clickDoubleObject(buttonNext, pageLeft, "L")
                clickDoubleObject(buttonNext, pageRight, "R")
                screenShotLR()

                if countObject(buttonNext, pageLeft) + countObject(buttonNext, pageRight) == 0 :
                    print("loop Break")
                    break
